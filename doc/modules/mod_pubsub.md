...
Allows to organise nodes in trees if `hometree` plugin is used.
...
Every node takes a place in a collection and is either a collection node (and have only sub-nodes) or a leaf node (contains only items).
...
| `[global, backends, mod_pubsub_db, create_node, ]` | histogram | Time to set the owner after node is created. |
