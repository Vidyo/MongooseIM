...
The double-bracket syntax is used because there can be multiple listeners of a given type, so technically for each listener type there is a TOML array of tables (subsections).
...
* **Default:** any IP address is allowed by default (0.0.0.0 for IPv4).
...
* **Default:** `4` unless the `ip_address` is an IPv6 one - then it is `6`
...
The options listed below can be set for `c2s`, `s2s` and `service` listeners to adjust their parameters.
...
This option specifies the types of incoming connections (e.g. XMPP, HTTP), for which MongooseIM is listening on the specified port numbers. By default there are no listeners defined, so MongooseIM would not accept any connection.
...
The recommended port number for a C2S listener is 5222.
...
The access rule that determines who is allowed to connect. By default the rule is `all`, what means that anyone can connect. The access rule referenced here needs to be defined in the `access` configuration section.
...
#### `listen.c2s.tls.verify_mode` - for `just_tls`
...
#### `listen.c2s.tls.disconnect_on_failure` - for `just_tls`
* **Syntax:** boolean
* **Default:** `true`
* **Example:** `tls.disconnect_on_failure = false`

Requires setting `tls.verify_mode`.  When set to `false`, it allows the client to connect even though the certificate verification failed. It is then up to the authentication layer to accept or reject the client connection. This behaviour mimcs the FastTLS one.
...
Path to the X509 PEM file with a CA chain that will be used to verify clients. Won't have any effect if `verify_peer` is not enabled.
...
* **Example:** `tls.ciphers = "[{cipher = "aes_25_gcm", key_exchange = "any", max = "aead", "prf = sha384"}]"`
...
#### `listen.c2s.tls.versions` - for `just_tls`
...
#### `listen.c2s.tls.crl_files` - for `just_tls`
...
#### `listen.c2s.tls.protocol_options` - for `fast_tls`
* **Syntax:** array of strings
...
The recommended port number for an S2S listener is 5269.
...
S2S connections do not use TLS encyprtion unless enabled with the `use_starttls` option in the `s2s` section.
...
Path to the X509 PEM file with a CA chain that will be used to verify the connecting XMPP servers (acting as clients here). Won't have any effect if `verify_peer` is not enabled.
...
Determines who is allowed to connect to the listener. By default the rule is `all`, what means that anyone can connect. The access rule referenced here needs to be defined in the `access` configuration section.
...
By default, when a component tries to connect and a registration conflict occures, the connection is dropped with the following error:
...
There are following options for each of the HTTP listeners:
...
* **Syntax:** each handler is specified in a subsection starting with `[[listen.http.handlers.type]]` where `type` is one of the allowed handler types, handling different connection types, e.g.
    ...
    * `mongoose_api_*`, `mongoose_client_api_*`, ... - for REST API.
    ...
    The double-bracket syntax is used because there can be multiple handlers of a given type, so technically for each type there is a TOML array of tables (subsections).
