...
#### `listen.c2s.access`
* **Syntax:** string, ACL name
* **Default:** `"all"`
* **Example:** `access = "c2s"`

The ACL that determines who is allowed to connect. By default the rule is `all`, which means that anyone can connect. The ACL referenced here needs to be defined in the `access` configuration section.
...
The ACL that determines what traffic shaper is used to limit the incoming XMPP traffic to prevent the server from being flooded with incoming data.
The ACL referenced here needs to be defined in the `access` configuration section.
...
#### `listen.s2s.shaper`
* **Syntax:** string, name of the shaper ACL or `"none"`
* **Default:** `"none"` - no shaper
* **Example:** `shaper = "s2s_shaper"`

Name of the ACL that determines what traffic shaper is used to limit the incoming XMPP traffic to prevent the server from being flooded with incoming data. The ACL referenced here needs to be defined in the `access` config section and it should return the shaper name or the value `"none"`.
